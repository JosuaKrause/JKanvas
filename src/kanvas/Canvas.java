...
public class Canvas extends JComponent implements Refreshable {
  ...
  private final class CanvasContext implements Context {
    ...
    @Override
    public Rectangle2D toCanvasCoordinates(final Rectangle2D r) {
      final Point2D topLeft = toCanvasCoordinates(
          new Point2D.Double(r.getMinX(), r.getMinY()));
      final double w = toCanvasLength(r.getWidth());
      final double h = toCanvasLength(r.getHeight());
      return new Rectangle2D.Double(topLeft.getX(), topLeft.getY(), w, h);
    }
    ...
    @Override
    public Rectangle2D toComponentRectangle(final Rectangle2D r) {
      final Point2D topLeft = toComponentCoordinates(
          new Point2D.Double(r.getMinX(), r.getMinY()));
      final double w = toComponentLength(r.getWidth());
      final double h = toComponentLength(r.getHeight());
      return new Rectangle2D.Double(topLeft.getX(), topLeft.getY(), w, h);
    }
