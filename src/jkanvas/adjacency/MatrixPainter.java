...
public abstract class MatrixPainter<T> extends PainterAdapter {
  ...
  private final CellColor<T> cellColor;

  public MatrixPainter(final AdjacencyMatrix<T> matrix, final CellColor<T> cellColor) {
    this.matrix = Objects.requireNonNull(matrix);
    this.cellColor = Objects.requireNonNull(cellColor);
  }
  ...
  public void draw(final Graphics2D gfx, final KanvasContext ctx) {
    ...
    for(int row = 0; row < matrix.size(); ++row) {
      ...
      for(int col = 0; col < matrix.size(); ++col) {
        ...
        final T val = matrix.get(row, col);
        gfx.setColor(cellColor.getColorFor(row, col, val, sel));
        gfx.fill(rect);
        gfx.setColor(cellColor.getBorderColorFor(row, col, val, sel));
        gfx.draw(rect);
        x += w;
      }
      y += h;
